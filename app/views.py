...
def user_login(request):
    ...
    if user is not None:
        ...
        # Debugging information to print user's first and last name
...
@csrf_exempt  # Disable CSRF for this view (only if necessary)
def search_user(request):

    print("Request hit backend")  # This should show in the Django logs
    query = request.GET.get('query', '')
    print(f"Search query: {query}")
    query = request.GET.get('query', '')
    if query:
        users = CustomUser.objects.filter(username__icontains=query) | CustomUser.objects.filter(first_name__icontains=query) | CustomUser.objects.filter(last_name__icontains=query)
        user_data = [{'id': user.id, 'username': user.username, 'first_name': user.first_name, 'last_name': user.last_name} for user in users]
